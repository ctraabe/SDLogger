#include "tera_ranger.h"

static const uint8_t crc_table[] = {
  };

TeraRanger::TeraRanger(HardwareSerial &serial) :
  serial_(serial),
  rx_buffer_({ 0 }),
  rx_buffer_head_(0),
  packet_queue_({ { 0 } }),
  packet_queue_head_(0),
  packet_queue_tail_(0),
  crc_({ 0 })
{
}

void TeraRanger::Init(void)
{
  serial_.begin(115200);
}

void TeraRanger::ProcessIncoming(void)
{
  if (!serial_.available()) return;

  static uint length = 0;
  uint8_t byte = serial_.read();

  switch (rx_buffer_head_)
  {
    case 3:  // Sync char
      if (byte != SONAR_START)
        rx_buffer_head_ = 0;
      else
        rx_buffer_[rx_buffer_head_++] = byte;
      break;
    case 0:  // Sync char
      if (byte != IR_START) break;
    case 1:
    case 2:
    case 4:
    case 5:
      rx_buffer_[rx_buffer_head_++] = byte;
      break;
    case 6:
      crc_ = CRC8(rx_buffer_, 6);
      if (crc_ == byte) DecodeRx();
    default:
      rx_buffer_head_ = 0;
      break;
  }
}

uint8_t TeraRanger::CRC8(uint8_t * array, uint8_t len)
{
  uint16_t i;
  uint16_t crc = 0x0;

  while (len--)
  {
    i = (crc ^ *array++) & 0xFF;
    crc = (crc_table[i] ^ (crc << 8)) & 0xFF;
  }
  return crc & 0xFF;
}

void TeraRanger::DecodeRx(void)
{
  union U16Bytes temp;
  temp.bytes[0] = rx_buffer_[2];
  temp.bytes[1] = rx_buffer_[1];
  packet_queue_[packet_queue_head_].ir = temp.u16;
  temp.bytes[0] = rx_buffer_[5];
  temp.bytes[1] = rx_buffer_[4];
  packet_queue_[packet_queue_head_].sonar = temp.u16;
  packet_queue_[packet_queue_head_].crc = crc_;

  // Move the packet_queue head to the next slot.
  packet_queue_head_ = (packet_queue_head_ + 1) & kPacketQueuMask;
}
